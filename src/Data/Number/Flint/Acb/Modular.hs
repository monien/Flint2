...
and Jacobi theta functions. See @acb_elliptic.h \<acb-elliptic>@ for the
closely related elliptic functions and integrals.

In the context of this module, /tau/ or \(\tau\) always denotes an
element of the complex upper half-plane mathbb{H} = {z in mathbb{C} :
operatorname{Im}(z) > 0}. We also often use the variable \(q\),
...
bounds: if /tau/ is represented by an @acb_t@ whose content overlaps
